...
struct GuestHomeView: View {
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(spacing: 14) {
                    // ✅ نفس الألوان الأصلية + glassEffect فقط
                    RoundedRectangle(cornerRadius: 25)
                        .fill(isDark ? Color.black.opacity(0.18) : Color(hex: "#F6F6F6"))
                        .frame(width: 188, height: 214)
                        .overlay(RoundedRectangle(cornerRadius: 25).stroke(isDark ? Color(hex: "#8E8AC5").opacity(0.35) : Color.clear, lineWidth: 1))
                        .shadow(color: Color.black.opacity(isDark ? 0 : 0.25), radius: 4, x: 0, y: 4)
                        .overlay(ProgressRingView(percentage: 0, ringSize: 120, lineWidth: 10).environment(\.locale, Locale(identifier: "en_US")))
                ...
                HStack(spacing: 13) {
                    PrimaryButtonView(title: NSLocalizedString("create", comment: ""), style: .solid, action: onCreateChallenge)
                        .popoverTip(createChallengeTip, arrowEdge: .top)
                    PrimaryButtonView(title: NSLocalizedString("join", comment: ""), style: .muted, action: onJoinChallenge)
                }
                .padding(.horizontal, 23)
                .padding(.top, 59)
                .padding(.bottom, 40)
    ...
    private var backgroundView: some View {
        if isDark {
            LinearGradient(colors: [Color.black, Color.black, Color(hex: "#3E3C5E")], startPoint: .top, endPoint: .bottom).ignoresSafeArea()
        } else {
...
// ✅ Helper — يضيف glassEffect فقط على iOS 26+ بدون أي تغيير بالألوان
    ...
    var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 12) {
                ...
                .font(.system(size: 24, weight: .bold)).foregroundColor(Color(hex: "#377329"))
            ...
            Rectangle().fill(isDark ? Color.white.opacity(0.18) : Color(hex: "#D5D5D5")).frame(height: 1)
            ...
            VStack(spacing: 12) {
                ...
                .environment(\.locale, Locale(identifier: "en"))
                .font(.system(size: 24, weight: .bold)).foregroundColor(Color(hex: "#C57723"))
        ...
        .overlay(RoundedRectangle(cornerRadius: 25).stroke(isDark ? Color.white.opacity(0.18) : Color(hex: "#D5D5D5"), lineWidth: 1))
    }
}

#Preview("Light") { GuestHomeView(onSignIn: {}, onCreateChallenge: {}, onJoinChallenge: {}) }
#Preview("Dark") { GuestHomeView(onSignIn: {}, onCreateChallenge: {}, onJoinChallenge: {}).preferredColorScheme(.dark) }
