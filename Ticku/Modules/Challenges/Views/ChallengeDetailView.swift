...
struct ChallengeDetailView: View {
    ...
    var body: some View {
        ...
        .onChange(of: vm.timerDisplay) {
            if vm.timerDisplay == "00:00:00" && vm.challengeStartDate != nil {
                Task { await vm.completeChallenge(); showComplete = true }
            }
        ...
        } message: { Text(NSLocalizedString("give_up_message", comment: "")) }
        .onAppear { vm.startListening(currentUserId: authVM.currentUserId ?? ""); startPendingListener() }
        .onDisappear { pendingListener?.remove() }
        .task { await vm.saveInviteCode() }
        .alert(NSLocalizedString("edit_challenge_name", comment: ""), isPresented: $showNamePopup) {
            ...
            Button(NSLocalizedString("save", comment: "")) {
                if !editedName.isEmpty && editedName != vm.challenge.title { updateChallengeName(editedName) }
            }
        }
        .withErrorHandling()
    }
    ...
    private var soloLayout: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                if vm.timerDisplay != "START" { menuButton }
            }
    ...
    private var backgroundView: some View {
        if isDark {
            LinearGradient(colors: [Color(hex: "#000000"), Color(hex: "#3E3C5E")], startPoint: .top, endPoint: .bottom).ignoresSafeArea()
        } else {
            LinearGradient(colors: [Color.white, Color.white, Color(hex: "#341D71").opacity(0.69)], startPoint: .top, endPoint: .bottom).ignoresSafeArea()
        }
    }
    ...
    private var playersList: [Player] {
        vm.members.enumerated().map { index, member in
            Player(name: member.displayName, rank: index + 1, isMe: member.userId == authVM.currentUserId, completedTasks: member.tasksCompleted, totalTasks: member.tasksTotal)
        }
    }
    ...
    private var navBarSection: some View {
        HStack {
            Button(action: onBack) {
                Image(systemName: "chevron.backward").font(.system(size: 20, weight: .semibold)).foregroundColor(isDark ? .white : .black)
            }
            Spacer()
            if vm.timerDisplay == "START" { shareButton } else { menuButton }
        }
        .padding(.horizontal, 24).padding(.top, 16)
    }
    ...
    private var shareButton: some View {
        Button(action: shareChallenge) {
            Image(systemName: "square.and.arrow.up").font(.system(size: 20, weight: .semibold)).foregroundColor(isDark ? .white : .black)
        }
    }
    ...
    private var menuButton: some View {
        Button(action: { showMenu = true }) {
            ZStack(alignment: .topTrailing) {
                Image(systemName: "ellipsis").font(.system(size: 20, weight: .semibold)).foregroundColor(isDark ? .white : .black).rotationEffect(.degrees(90)).padding(4)
                if hasPendingChange { Circle().fill(Color.red).frame(width: 10, height: 10).offset(x: 4, y: -4) }
            }
        }
    ...
    private var timerSection: some View {
        ...
        // ✅ Liquid Glass على كارد العداد — شفاف عشان ما يتعارض مع الألوان
        ZStack {
        ...
        .liquidGlass(tint: isDark ? Color.white.opacity(0.12) : Color(hex: "#E8E2F8"), cornerRadius: 32)
        .onTapGesture {
            if vm.timerDisplay == "START" { Task { await vm.startChallenge() } }
        }
        ...
        Text("\(vm.members.count) \(NSLocalizedString("players_count", comment: ""))")
            ...
            .liquidGlass(tint: isDark ? Color(hex: "#1E3A5F").opacity(0.7) : Color(hex: "#E4E8FF"), cornerRadius: 20)
            .environment(\.locale, Locale(identifier: "en_US"))
    }
    ...
    private var titleSection: some View {
        HStack(spacing: 8) {
            Text(vm.challenge.title).font(.system(size: 26, weight: .black)).foregroundColor(isDark ? .white : .black)
            if vm.timerDisplay == "START" {
                Button(action: { editedName = vm.challenge.title; showNamePopup = true }) {
                    Image(systemName: "pencil").font(.system(size: 16, weight: .medium)).foregroundColor(isDark ? .white.opacity(0.6) : Color(hex: "#888888"))
                }
            }
        }
    }
    ...
    private func memberGridItem(member: ChallengeMember, index: Int) -> some View {
        ...
        if isMe {
            Button(action: { onMyTasks(vm.challenge) }) { MemberCard(member: member, rank: index + 1, isMe: true) }
                .buttonStyle(.plain).popoverTip(streakTip, arrowEdge: .top)
        } else {
    ...
    private func startPendingListener() {
        ...
        pendingListener = db.collection("challenges").document(cid)
            .addSnapshotListener { snap, _ in hasPendingChange = snap?.data()?["pendingRuleChange"] != nil }
    }
...
private struct MemberCard: View {
    ...
    @Environment(\.colorScheme) private var colorScheme
    private var progress: Double { min(max(member.progressPercent / 100.0, 0), 1) }
    private var isDark: Bool { colorScheme == .dark }
    private var progressColor: Color { isDark ? Color(hex: "#8E8AC5") : Color(hex: "#341D71") }
    private var trackColor: Color { isDark ? Color(hex: "#8E8AC5").opacity(0.28) : Color(hex: "#D9D5F0") }

    var body: some View {
        VStack(spacing: 8) {
            progressAvatar.padding(.top, 16)
        ...
        // ✅ Liquid Glass على كارد العضو — شفاف عشان يعكس خلفية التطبيق بدل ما يتعارض معها
        .liquidGlass(
            tint: isDark ? Color.black.opacity(0.35) : Color(hex: "#F6F6F6").opacity(0.65),
            cornerRadius: 24
        )
    }
    ...
    private var progressAvatar: some View {
        ZStack {
            Circle().stroke(trackColor, lineWidth: 7).frame(width: 82, height: 82)
            if progress > 0 {
            ...
            Circle().fill(isDark ? Color(hex: "#3A3A3A") : Color(hex: "#E8E4F0")).frame(width: 68, height: 68)
            Image(systemName: "person.fill").font(.system(size: 32, weight: .regular)).foregroundColor(isDark ? Color(hex: "#FFE8CB") : Color(hex: "#341D71"))
        }
...
private struct EmptySlotCard: View {
    var body: some View {
        VStack(spacing: 8) {
            ZStack {
                Circle().stroke(Color(hex: "#D9D5F0"), lineWidth: 7).frame(width: 82, height: 82)
                Circle().fill(Color(hex: "#E8E4F0")).frame(width: 68, height: 68)
            ...
            Text(NSLocalizedString("invite", comment: "")).font(.system(size: 14, weight: .black)).foregroundColor(Color(hex: "#A89DD4"))
            Spacer(minLength: 4)
        }
        .frame(height: 210).frame(maxWidth: .infinity)
        .liquidGlass(tint: Color.white.opacity(0.15), cornerRadius: 24)
    }
}

#Preview("Light") {
    NavigationStack {
        ChallengeDetailView(challenge: Challenge(id: "preview", title: "let's do it", description: "", createdBy: "uid", startDate: Date().addingTimeInterval(60), endDate: Calendar.current.date(byAdding: .day, value: 3, to: Date())!, status: "active", memberCount: 1, createdAt: Date(), memberIds: ["uid"]))
