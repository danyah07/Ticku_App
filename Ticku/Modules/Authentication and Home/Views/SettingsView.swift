...
struct SettingsView: View {
    ...
    private var nameField: some View {
        HStack(spacing: 6) {
            TextField(
                NSLocalizedString("your_name", comment: ""),
                text: $vm.displayName
            )
            .font(
                .system(
    ...
    private var settingsCard: some View {
        VStack(alignment: .leading, spacing: 13) {
            ...
            .settingsCardStyle(isDark: isDark)
        }
        .frame(width: 349)
    }
    ...
    private var privacyCard: some View {
        Button {
            ...
            .shadow(
                color: Color.black.opacity(isDark ? 0 : 0.06),
                radius: 5,
                x: 0,
                y: 2
            )
    ...
    private var deleteCard: some View {
        Button {
            ...
            HStack(spacing: 12) {
                ZStack {
                    Circle()
                        .fill(Color.red.opacity(isDark ? 0.18 : 0.10))
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 25)
                    .stroke(
                        Color.red.opacity(isDark ? 0.20 : 0.14),
                        lineWidth: 1
                    )
            )
            .shadow(
                color: Color.black.opacity(isDark ? 0 : 0.04),
                radius: 4,
                x: 0,
                y: 2
            )
        ...
        VStack {
            ...
            if let success = vm.successMessage {
                Text(success)
                    ...
                    .padding(.vertical, 10)
                    .background(
                        Color.ticku.doneGreen.opacity(0.9)
                    )
                    .clipShape(Capsule())
                    .padding(.bottom, 100)
                    .onTapGesture {
            ...
            if let error = vm.errorMessage {
                Text(error)
                    ...
                    .padding(.vertical, 10)
                    .background(
                        Color.ticku.errorRed.opacity(0.9)
                    )
                    .clipShape(Capsule())
                    .padding(.bottom, 100)
                    .onTapGesture {
...
private extension View {
    func settingsCardStyle(isDark: Bool) -> some View {
        self
            .frame(width: 349)
            .background(
                isDark
...
// MARK: - Delete Account Delegate

private final class DeleteAccountDelegate:
    NSObject,
    ...
    func authorizationController(
        ...
        print(
            "❌ Re-auth cancelled or failed: \(error.localizedDescription)"
        )
    }
}
